...
def _occupies_slot(record: Dict[str, Any]) -> bool:
    ...
    return (
        ...
        or record.get("_runner_settled") is False
    )
...
def status_snapshot(*, origin_ui_session_id: str = "", parent_session_id: str = "") -> Dict[str, Any]:
    ...
    with _records_lock:
        records = [
            ...
            if (
                not origin_ui_session_id
                and not parent_session_id
                or origin_ui_session_id
                and r.get("origin_ui_session_id") == origin_ui_session_id
    ...
    return {
        "active_tasks": sum(
            b["task_count"] for b in batches if b["status"] in {"running", "finalizing"}
        ),
        "stalled_tasks": sum(
            b["task_count"]
            for b in batches
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    with _records_lock:
        active_slots = {
            r.get("slot_key") or r["delegation_id"]
            for r in _records.values()
            if _occupies_slot(r)
        }
