...
voice:
  ...
  beep_enabled: true            # Play recording-start and STT-complete cues in CLI voice mode
...
delegation:
  ...
  resource_routing:
    escalation:
      enabled: true                         # Allow user-authorized premium child proposals; never auto-select them.
      authorization_ttl_seconds: 300        # One-use approval lifetime (minimum 30 seconds).
      cooldown_seconds: 120                 # Per-session delay between proposals.
      max_requests_per_session: 3           # Bound premium-route approval prompts.
```
...
**User-authorized escalation:** Hermes can propose a more capable route for one bounded `openai-codex` child, but cannot select it from model prose alone. Use the bare model ID returned by the approval, or the exact `openai-codex/<bare-model>` form; a qualified ID for another provider is rejected rather than silently retargeted. The prompt offers **Approve this child**, **Use lower model**, **Decline**, or **Defer**. Approval creates an opaque capability bound to the active session, target model, and exact child scope; it expires, can be used once, never changes the parent session's model, and cannot create a persistent allowlist. Sol needs explicit user approval; automatic `judgment` routing remains Terra. Astra, Spark, and 900k routes are also always explicit. A 900k proposal also needs an isolated-context estimate at least as large as `resource_routing.large_context_trigger_tokens`.

Approved escalation and automatic-route metadata report separate control planes: configured `max_concurrent_children`, occupied async slots, the per-child timeout, model authorization, selected isolated context tier, and provider quota. Provider quota stays `not_observed` unless an actual provider response reports a quota/rate-limit failure; governor or routing advice is not such a failure.
