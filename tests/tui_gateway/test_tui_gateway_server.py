...
def test_voice_status_reports_effective_recording_policy(monkeypatch):
    ...
    for cfg, expected in (
        ...
        response = _dispatch_sync(
            {"id": "voice-status-policy", "method": "voice.toggle", "params": {"action": "status"}}
        )["result"]
...
def test_voice_record_start_defaults_to_silence_and_reports_recording_contract(monkeypatch):
    ...
    assert emitted[-1][1]["silence_remaining_seconds"] == 3.2
    captured["on_cutoff"]("hard_limit")
    assert emitted[-1][1]["cutoff_reason"] == "hard_limit"
    assert emitted[-1][1]["state"] == "transcribing"
    assert server._voice_capture_owner == "record"
    captured["on_error"]("Parakeet unavailable")
    assert emitted[-1] == ("voice.transcript", {"error": "Parakeet unavailable"})
...
def test_voice_record_start_refuses_second_microphone_owner(monkeypatch):
    starts = []
    monkeypatch.setenv("HERMES_VOICE", "1")
    monkeypatch.setattr(server, "_voice_capture_owner", "full_duplex")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.voice",
        types.SimpleNamespace(
...
def test_tts_stream_vad_barge_in_cuts_pipeline_and_submits_capture(monkeypatch, tmp_path):
    ...
    assert any(
        event == "voice.status"
        and payload.get("silence_remaining_seconds") == 4.2
        and payload.get("capture_kind") == "barge_in"
