...
class TestDetectAudioEnvironment:
    ...
    def test_wsl_without_pulse_and_no_fallback_blocks_voice(self, monkeypatch, tmp_path):
        """WSL without forwarded sound and without TTS fallback should block voice mode."""
        monkeypatch.delenv("SSH_CLIENT", raising=False)
        monkeypatch.delenv("SSH_TTY", raising=False)
        monkeypatch.delenv("SSH_CONNECTION", raising=False)
        monkeypatch.delenv("PULSE_SERVER", raising=False)
    ...
    def test_wsl_with_pipewire_remote_allows_voice(self, monkeypatch, tmp_path):
        """WSL with PIPEWIRE_REMOTE set should not require PULSE_SERVER."""
        monkeypatch.delenv("SSH_CLIENT", raising=False)
        monkeypatch.delenv("SSH_TTY", raising=False)
        monkeypatch.delenv("SSH_CONNECTION", raising=False)
        monkeypatch.delenv("PULSE_SERVER", raising=False)
        ...
        def fake_open(file, *args, **kwargs):
            if file == "/proc/version":
                return real_open(str(proc_version), *args, **kwargs)
            return real_open(file, *args, **kwargs)
