    ...
    run: (arg, ctx) => {
      ...
      ctx.gateway.rpc<VoiceToggleResponse>('voice.toggle', payload).then(
        ctx.guarded<VoiceToggleResponse>(r => {
          ...
          if (action === 'silence') {
            ctx.transcript.sys(
              r.recording_mode === 'manual'
                ? 'Voice silence endpoint: off (manual stop)'
                : `Voice silence endpoint: ${r.silence_duration_seconds ?? 5}s`
            )
          ...
          if (action === 'status') {
            ...
            ctx.transcript.sys(`  Input:      ${r.input_mode === 'dictation' ? 'dictation draft' : 'immediate submit'}`)
            ...
            ctx.transcript.sys(
              `  Silence:    ${r.recording_mode === 'silence' ? `${r.silence_duration_seconds ?? 5}s` : 'off'}`
            )
